...
public class GeneradorDeRayos : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, dirTargetDirecto, out hitDirect, Mathf.Infinity))
        {
            ...
            {
                distanciaDirecta = hitDirect.distance;
                anguloDirecto = 180 * Mathf.Atan(Mathf.Abs(dirTargetDirecto.y) / Mathf.Abs(dirTargetDirecto.x)) / Mathf.PI;
        ...
        if (Physics.Raycast(transform.position, dirVerticeVarible, out hit1, Mathf.Infinity))
        {
            ...
            if (hit1.transform.gameObject.CompareTag("VerticeVariable"))
            {
                ...
                {
                    disInd1 = dirVerticeVarible.magnitude;
                    disInd2 = dirTargetIndirecto.magnitude;
                    anguloIndirecto = 180 * Mathf.Asin(Mathf.Abs(dirTargetIndirecto.y) / disInd2) / Mathf.PI;
            ...
            distanciaIndirecta = disInd1 + disInd2;
